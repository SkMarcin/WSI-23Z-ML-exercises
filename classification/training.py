...
def get_fallout(dictionary, classes):
    precisions = []
    for classifier in classes:
        false_positives = 0
        total = 0
        for key in dictionary:
            if key[0] != classifier:
                if key[1] == classifier:
                    false_positives += dictionary[key]
                total += dictionary[key]
        if total > 0:
            precisions.append(false_positives/total)
    return sum(precisions) / len(precisions)

def get_f1score(dictionary, classes):
    precision = get_precision(dictionary, classes)
    recall = get_recall(dictionary, classes)
